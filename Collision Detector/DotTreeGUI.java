...
public class DotTreeGUI extends DrawingGUI {
	...
	private static final Color[] rainbow = {Color.RED, Color.ORANGE, Color.YELLOW, Color.GREEN, Color.BLUE, Color.MAGENTA}; // to color different levels differently
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') { //add
			...
			Dot dot = new Dot(x, y);
			if (tree != null) tree.insert(dot);
			else tree = new PointQuadtree<>(dot, 0, 0, width, height);
		}
		else if (mode == 'q') { //query
			// Set "found" to what tree says is near the mouse press
			tree.findInCircle(mouseX, mouseY, mouseRadius);
//			for (Dot d : tree.findInCircle(mouseX, mouseY, mouseRadius)) {
...
//			}
	...
	private void test2() {
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(175,331), 0,0,800,600); // start with A
		tree.insert(new Dot(196,311)); // B
		tree.insert(new Dot(285,424)); // C
		tree.insert(new Dot(282,422)); // D
		tree.insert(new Dot(321,408)); // E
		tree.insert(new Dot(330,401)); // F

		int bad = 0;
		bad += testFind(175,331,10,5,4,1); //tests at root node
		bad += testFind(186,321,10,5,4,0); //tests right between two nodes
		bad += testFind(450,410,10,6,4,0); //tests at child node
