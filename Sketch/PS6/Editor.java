...
public class Editor extends JFrame {	
	...
	private JComponent setupGUI() {
		...
		// Editor.Mode: draw, move, recolor, or delete
	...
	public synchronized void drawSketch(Graphics g) {
		for (Shape shape : sketch.map.values()) {
			shape.draw(g);
		}
		if (curr != null) {
			curr.draw(g);
	 ...
	 */
	private synchronized void handlePress(Point p) {
		//draw mode
		if (mode == Mode.DRAW) {
			drawFrom = p;
		...
		//move mode
		if (mode == Mode.MOVE) {
			movingId = sketch.findID((int) p.getX(), (int) p.getY());
			if (movingId != -1) {
					moveFrom = p;
				}
			}
		...
		if (mode == Mode.RECOLOR) {
			Integer localId = sketch.findID((int) p.getX(), (int) p.getY());
			if (localId != -1) {
				comm.send("recolor " + color.getRGB() + " " + localId);
			}
		}
		//delete mode
		if (mode == Mode.DELETE) {
	 ...
	 */
	private synchronized void handleDrag(Point p) {
		...
		if (mode == Mode.DRAW) {
			if (shapeType.equals("ellipse")) {
				...
				((Polyline) curr).marker((int) p.getX(), (int) p.getY());
			}
			repaint();
		}
		// In moving mode, shift the object and keep track of where next step is from
		if (mode == Mode.MOVE && moveFrom != null) {
			int changeX = (int) p.getX() - (int) moveFrom.getX();
			int changeY = (int) p.getY() - (int) moveFrom.getY();
			comm.send("move " + sketch.map.get(movingId).toString() + " " + changeX + " " + changeY + " " + movingId);
			moveFrom = p;
		}
	}
